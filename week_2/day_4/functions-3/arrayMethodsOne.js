...
// CODE HERE
const postTaxPricesOneLine = prices.map(price => (price * .07) + price )
 const postTaxPrices = prices.map( price => {
  let total = (price * .07) + price
  return total
 });

//  console.log(postTaxPricesOneLine)
...
let totaledOrder = orders.map(obj => obj.price + (obj.tax * obj.price))

// console.log(totaledOrder)
...
let bobsTotal = purchases.filter(obj => obj.owner === 'Bob').reduce((acc,obj)=> acc + obj.price , 0 )
